...
import { Basket, User } from './basketModel';
...
export async function addItemToBasket(req: Request, res: Response) {
  ...
  let modelMgr = new ModelManager(BASKET_FILE);
  let baskets: Basket[] = await modelMgr.getAll() as Basket[];
  const basketIndex = baskets.findIndex(basket => basket.basketId === basketId);   // finds the index of the basket in the baskets array 
  if (basketIndex !== -1) {
    const newItem = { itemId, productId, name, quantity };   // creates a new item object 
...
export async function updateItemQuantity(req: Request, res: Response) {
    ...
    let modelMgr = new ModelManager(BASKET_FILE);
    let baskets: Basket[] = await modelMgr.getAll() as Basket[];
    const basketIndex = baskets.findIndex(basket => basket.basketId === basketId);
    if (basketIndex !== -1) {
      const itemIndex = baskets[basketIndex].items.findIndex(item => item.itemId === itemId);   
